...
> **A Rust-native JavaScript build tool for SPAs and Module Federation**, with SWC + LightningCSS, HMR, and built-in supply-chain security. Requires the shipped `lunx_native` binary (one copy under `dist/`). JS docs footprint is kept ≤1.6MB; the native binary is ~15MB and is required for Rust speed.
